function stampa() {
    ...
    let mezziPosseduti = mezzi.length > 0 ? mezzi.join(", ") : "Nessuno";
    let materiePreferite = materie.length > 0 ? materie.join(", ") : "Nessuna";

    let generazione = "";
    switch (true) {
        case eta <= 12: generazione = "Generazione Alpha"; break;
        case (eta >= 13 && eta <= 28): generazione = "Generazione Z"; break;
        case (eta >= 29 && eta <= 44): generazione = "Millennials"; break;
        case (eta >= 45 && eta <= 60): generazione = "Generazione X"; break;
        default: generazione = "Boh!";
    }
...
// Funzione per salvare array di persone nel localStorage e aggiungere gli indici nell'URL
function salvaPersone() {
    const persone = [
        { nome: "Gino", cognome: "Latilla", eta: 32 },
        { nome: "Pino", cognome: "Rossi", eta: 42 }
    ];

    // Salvataggio nel localStorage
    localStorage.setItem("persone", JSON.stringify(persone));

    // Creazione indici per l'URL
    const indici = persone.map((_, i) => (i + 1).toString().padStart(2, "0")).join(",");
    
    // Modifica dell'action con query string
    const form = document.forms["frmMaschera"];
    form.action = "destinatarioCreaTabella.html?indici=" + indici;
}
